...
# MicroGridsPy real path
MICROGRIDSPY_DIR = PROJECT_ROOT / "MicroGridsPy-Development_Linopy-2" / "microgridspy"
if str(MICROGRIDSPY_DIR) not in sys.path:
    sys.path.insert(0, str(MICROGRIDSPY_DIR))
...
def worker_init():
    global run_module
    # Add MicroGridsPy directory to path (for config module)
    mgpy_root = str(PROJECT_ROOT / "MicroGridsPy-Development_Linopy-2")
    if mgpy_root not in sys.path:
        sys.path.insert(0, mgpy_root)
...
def main():
    ...
    # Choose number of workers - use SLURM/SGE allocation or all CPUs
    # SGE uses $NSLOTS, SLURM uses $SLURM_CPUS_PER_TASK
    num_workers = int(os.environ.get("NSLOTS", 
                      os.environ.get("SLURM_CPUS_PER_TASK", os.cpu_count() or 1)))

    print(f"Starting orchestrator with {num_workers} workers")
    ...
    with ProcessPoolExecutor(max_workers=num_workers, initializer=worker_init) as executor:
        ...
        for fut in as_completed(futures):
            try:
                ...
                completed += 1
                if completed % 50 == 0:
                    print(f"Progress: {completed}/{len(rows)} clusters completed")
            except Exception as e:
                failed += 1
    ...
    print(f"\n{'='*60}")
    print(f"Total execution time: {(end - start)/3600:.2f} hours")
