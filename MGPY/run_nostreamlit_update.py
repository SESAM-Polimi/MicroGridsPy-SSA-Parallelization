...
# Ensure MicroGridsPy root is on path (for both 'config' and 'microgridspy' imports)
MICROGRIDSPY_ROOT = PROJECT_ROOT / "MicroGridsPy-Development_Linopy-2"
if str(MICROGRIDSPY_ROOT) not in sys.path:
...
def run_yaml(yaml_path: Union[str, Path]):
    ...
    # School weights path resolved relative to project root (Data_sheet/School_weights.csv)
    school_weights_path = PROJECT_ROOT / "Data_sheet" / "School_weights.csv"
    ...
    # Ensure fresh solver log each run for single cluster
    try:
        if log_path.exists():
            log_path.unlink()
    except Exception:
    ...
    try:
        ...
        battery_losses = total_battery_inflow - total_battery_outflow
        ...
        if conversion_losses_var is not None:
            total_losses += conversion_losses_var.sum().values.item() / 1e6
        total_losses += battery_losses
